    ...
    for (int len = 2; len <= slen; len++) {
        while (1) {
            ...
            for (int right = left + 1; right < left + len; right++) {
                ...
                int flag = 0;
                int consider = vnProductions_cnt % 8;
                for (int vn_index = 0; vn_index < vnProductions_cnt - consider; vn_index += 8) {
                    ...
                    VnProduction tmp[8] = {
                        ...
                        vnProductions[vn_index + 4],
                        vnProductions[vn_index + 5],
                        vnProductions[vn_index + 6],
                        vnProductions[vn_index + 7]
                    };
                    ...
                    dp[left][left+len-1][tmp[4].parent] += dp[left][right-1][tmp[4].child1] * dp[right][left+len-1][tmp[4].child2];
                    dp[left][left+len-1][tmp[5].parent] += dp[left][right-1][tmp[5].child1] * dp[right][left+len-1][tmp[5].child2];
                    dp[left][left+len-1][tmp[6].parent] += dp[left][right-1][tmp[6].child1] * dp[right][left+len-1][tmp[6].child2];
                    dp[left][left+len-1][tmp[7].parent] += dp[left][right-1][tmp[7].child1] * dp[right][left+len-1][tmp[7].child2];
                    if (!flag && (dp[left][left+len-1][tmp[0].parent] != 0 ||
                            ...
                            dp[left][left+len-1][tmp[4].parent] != 0 ||
                            dp[left][left+len-1][tmp[5].parent] != 0 ||
                            dp[left][left+len-1][tmp[6].parent] != 0 ||
                            dp[left][left+len-1][tmp[7].parent] != 0)) {
                        flag = 1;
                    }
                }
                if (consider) {
                    for (int i = 0; i < consider; i++) {
                        VnProduction tmp = vnProductions[i + vnProductions_cnt - consider];
                        dp[left][left+len-1][tmp.parent] += dp[left][right-1][tmp.child1] * dp[right][left+len-1][tmp.child2];
                        if (!flag && dp[left][left+len-1][tmp.parent]) {
                            flag = 1;
                        }
                    }
                }
