...
int main() {
    ...
    for (long long thread_index = 0; thread_index < THREAD_NUM; thread_index++) {
    ...
    for (int len = 2; len <= slen; len++) {
        while (1) {
            ...
            for (int right = left + 1; right < left + len; right++) {
                if (!dp[left][right-1][MAX_VN_NUM] || !dp[right][left+len-1][MAX_VN_NUM]) {
                    continue;           
                }
                int flag = 0;
                int consider = vnProductions_cnt % 4;
                for (int vn_index = 0; vn_index < vnProductions_cnt - consider; vn_index += 4) {
